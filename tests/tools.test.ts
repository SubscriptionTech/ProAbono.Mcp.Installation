...
/**
 * Every generator tool records the step it generated in `.proabono/installation.json`, in the
 * project it is run in -- which is this repository while the suite runs. This file therefore runs
 * from a temporary directory; `node --test` gives each file its own process, so the change is
 * local to this one. The state file itself is asserted in `insite.test.ts`, against a root it
 * creates on purpose.
 */
process.chdir(mkdtempSync(join(tmpdir(), "proabono-tools-")));
...
describe("the generator tools", () => {
  ...
  // The gap this used to announce is closed. What the output must still carry is the endpoint the
  // resynchronization is called by, and the fact that the ceiling has not stopped mattering.
  it("wires the resynchronization, and keeps the ceiling that bounds a revoked right", async () => {
    const { client } = await connect([{ body: { TotalItems: 0, Items: [] } }]);
    ...
    assert.match(answer, /scaffold_notification_endpoint/);
    assert.match(answer, /maximum TTL|ceiling/i);
    assert.ok(
      !/not generated by this version/i.test(answer),
      "the output still announces a gap that has been closed",
    );
  });
