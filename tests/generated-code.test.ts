...
describe("generated rights-synchronization code", () => {
  ...
  // The statement of absence this used to carry is gone with the gap it described. What replaced
  // it must keep the two facts a developer acts on: which reference identifies the customer, and
  // that the ceiling is still load-bearing while the webhook is not validated.
  it("states what the resynchronization is wired to, and that the ceiling still applies", () => {
    assert.match(RESYNC_WIRING, /scaffold_notification_endpoint/);
    assert.match(RESYNC_WIRING, /ReferenceCustomer/);
    assert.match(RESYNC_WIRING, /CustomerBuyer/);
    assert.match(RESYNC_WIRING, /maximum TTL|ceiling/i);
    assert.ok(
      !/not generated by this version/i.test(RESYNC_WIRING),
      "the gap is closed; the text must not still announce it",
    );
  });

  it("generates the resynchronization itself in every stack", () => {
    for (const stack of STACKS) {
      const module = rightsModule(stack);
      assert.match(
        module,
        /refreshEntitlements|refresh_entitlements|RefreshAsync/,
        `${stack} generates no resynchronization for the notification endpoint to call`,
      );
      // It must re-read, not patch: the cached entry is dropped whatever its expiry says.
      assert.match(
        module,
        /whatever its expiry|regardless of expiry|drop the cached entry/i,
        `${stack} does not say the resync ignores the entry's expiry`,
      );
    }
  });
});
